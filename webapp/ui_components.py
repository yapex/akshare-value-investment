...
def render_main_content() -> None:
    ...
    本系统提供四大财务报表的查询和深度分析功能：

    - **财务指标**: 核心财务指标和比率分析
    - **资产负债表**: 企业资产、负债和所有者权益状况
    - **利润表**: 企业收入、成本和利润情况
    - **现金流量表**: 企业现金流入流出分析
    ...
    2. 选择查询时间范围
    3. 点击"查询财务数据"按钮
    4. 查看报表数据，点击指标名称进行深度分析

    ### 💡 功能特色

    - **✅ 智能缓存**: 30天数据缓存，提升查询速度
    - **✅ 原始数据**: 保留akshare原始字段，数据完整可靠
    - **✅ 窄表格式**: 专业财务格式，百万元单位，小数点后2位
    - **✅ 交互图表**: 点击指标名称生成可视化分析图表
    - **✅ 深度分析**: 柱状图显示数值，折线图显示增长率
