...
- 详细说明部分不能丢，必须包含计算过程和具体数值
- 每项分析以结论开头，换行后加上详细说明部分
- **【重要】**对于所有可计算项，在详细说明部分的最后，必须单独列出"**原始数据计算过程**"，格式如下：
  ```
  原始数据计算过程：
  - 字段A：X（来源：资产负债表/利润表/现金流量表）
  - 字段B：Y（来源：资产负债表/利润表/现金流量表）
  - 计算公式：[具体计算公式]
  - 计算结果：[具体数值]
  ```
- Emoji建议：
...
原始数据计算过程：
- 货币资金：4.64亿（来源：资产负债表）
- 交易性金融资产：1.69亿（来源：资产负债表）
- 短期借款：6005.07万（来源：资产负债表）
- 长期借款：11.64亿（来源：资产负债表）
- 计算公式：(货币资金 + 交易性金融资产) ÷ (短期借款 + 长期借款) × 100%
- 计算结果：(4.64亿 + 1.69亿) ÷ (6005.07万 + 11.64亿) × 100% = 51.7%
```
...
- 计算结果：(13.19亿 - 8.96亿) ÷ 13.19亿 × 100% = 32.1%
```

#### 示例3：净利润含金量
```
原始数据计算过程：
- 经营活动产生的现金流量净额：4.07亿（来源：现金流量表）
- 归属于母公司所有者的净利润：7088.25万（来源：利润表）
- 计算公式：经营活动产生的现金流量净额 ÷ 归属于母公司所有者的净利润 × 100%
- 计算结果：4.07亿 ÷ 7088.25万 × 100% = 574.6%
```
