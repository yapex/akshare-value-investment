...
class FinancialQueryRequest(BaseModel):
    ...
    fields: Optional[List[str]] = Field(None, description="需要返回的字段列表")
    start_date: Optional[str] = Field(None, description="开始日期，YYYY-MM-DD格式")
    end_date: Optional[str] = Field(None, description="结束日期，YYYY-MM-DD格式")
